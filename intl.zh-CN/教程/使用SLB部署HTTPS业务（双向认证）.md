# 使用SLB部署HTTPS业务（双向认证） {#concept_mkk_3j5_vdb .concept}
...
3.  运行以下命令生成私钥key文件。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4133/15382929492750_zh-CN.png)
...
4.  运行以下命令并按命令后的示例提供需要输入的信息，然后回车，生成证书请求csr文件。
    ...
    **说明：** Common Name请输入您的负载均衡服务的域名。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4133/15382929492753_zh-CN.png)
...
3.  运行以下命令为客户端key创建一个证书签名请求csr文件。
    ...
    **说明：** A challenge password是客户端证书口令\(请注意将它和`client.key`的口令区分开，本教程设置密码为test\)，可以与服务器端证书或者根证书口令一致。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4133/15382929492757_zh-CN.png)
...
4.  运行以下命令使用步骤二中的CA Key为刚才的客户端key签名。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4133/15382929492758_zh-CN.png)
...
5.  运行以下命令将证书转换为大多数浏览器都能识别的PKCS12文件。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4133/15382929492759_zh-CN.png)
...
6.  运行以下命令查看生成的客户端证书。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4133/15382929502760_zh-CN.png)
...
3.  配置负载均衡实例，单击**立即购买**完成支付。
    ...
    本操作中网络类型选择**公网**，地域选择**华东1（杭州）**，详细配置信息参考[创建负载均衡实例](intl.zh-CN/用户指南（旧版，即将下线）/负载均衡实例/创建实例.md#)。
...
7.  在创建证书页面，完成如下配置后，单击**确定**。
    ...
    -   **证书内容和私钥**：复制您的服务器证书内容和私钥。
        ...
        **说明：** 在复制内容前，您可以单击**导入样式**，查看正确的证书和私钥格式。更多详细信息查看[证书要求](intl.zh-CN/用户指南（旧版，即将下线）/证书管理/证书要求.md#)。
...
2.  在IE浏览器中导入下载的客户端证书。
    ...
    2.  单击**内容**页签，然后单击**证书**，导入下载的客户端证书。在导入证书时需要输入在步骤三时生成PKCS12文件的密码。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4133/15382929502781_zh-CN.png)
...
4.  在协议&监听页签下，配置监听。
    ...
    -   **监听端口**：443
    -   **调度算法**：轮询（RR）
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16604/153829295010035_zh-CN.png)

5.  单击**下一步**，在SSL证书页签下，配置SSL证书信息，启用双向认证。

    -   **服务器证书**：选择已上传的服务器证书。
    -   **CA证书**： 选择已上传的CA证书。
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15659/15382929507331_zh-CN.png)
...
2.  在浏览器中，输入负载均衡的公网服务地址，当提示是否信任客户端证书时，选择信任。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4133/15382929502786_zh-CN.png)

3.  刷新浏览器，您可以观察到请求在两台ECS服务器之间转换。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15658/15382929507447_zh-CN.png)

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15658/15382929507448_zh-CN.png)
